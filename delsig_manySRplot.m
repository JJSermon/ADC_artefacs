function [HHmean,HHstd] = delsig_manySRplot(StimF,StimA,bipolar,dt_a,pw,Pfreqs,Threshold,Trials,Time)
...
if isempty(Pfreqs)
    Pfreqs = linspace(250,5000,10000); % linspace(250,5000,10000) was used for figure in publication
end
...
freqs = 0:0.1:2*StimF;
PVect = unique(round(1./Pfreqs,6));

HalfHarm_rec = NaN(length(PVect),Trials);
Harm1_rec = NaN(length(PVect),Trials);
...
for i_P = 1:length(PVect)
    P = PVect(i_P); Plength = round(P/dt_a); SR = 1/P;
    for i_Tr = 1:Trials
        clc
        disp([num2str(100*((i_P-1)*Trials+i_Tr)/(length(PVect)*Trials)) '% complete'])
...
%         PSD = pwelch(StimSeq_d-mean(StimSeq_d),length(StimSeq_d),[],freqs,SR);
        ...
        HalfHarm_rec(i_P,i_Tr) = PSD(HH_ind(1));
        Harm1_rec(i_P,i_Tr) = PSD(H1_ind(1));
...
figure
hold on
plot(1./PVect,HHmean,'b')
patch([1./PVect fliplr(1./PVect)],[HHmean+HHstd fliplr(HHmean-HHstd)],'b','EdgeColor','none','FaceAlpha',0.4);
xlim([Pfreqs(1) Pfreqs(end)])
